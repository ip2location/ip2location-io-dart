...
class DomainWhois {
  ...
  static const String _errorWhois = 'DomainWhois lookup error.';

  static const List<String> _tlds = [
    '.aaa',
    '.aarp',
    '.abarth',
    '.abbott',
  ...
  Future<Map<String, dynamic>> lookUp(String domain) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (rawJson.containsKey('error')) {
        final errorObj = rawJson['error'];
        if (errorObj is Map && errorObj.containsKey('error_message')) {
          throw Exception(errorObj['error_message']);
        }
      }
    }
    throw Exception(_errorWhois);
  }
  ...
  /// Extract just the domain extension (TLD) from a URL or domain string.
  String toDomainExtension(String str) {
    final domain = toDomainName(str);
    final dotIndex = domain.indexOf('.');
    if (dotIndex == -1) return '';
    return domain.substring(dotIndex); // everything from first '.' onward
  }

  /// Find the longest matching TLD in our list for a given domain.
  String? _findTld(String domain) {
    final lower = domain.toLowerCase();
    for (final tld in _tlds) {
      if (lower.endsWith('.$tld') || lower == tld) return tld;
    }
    return null;
  }
}
